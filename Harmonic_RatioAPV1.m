...
%ff = fr(fr > harmonic_n(2)-0.2 & fr< harmonic_n(2)+0.2);
 %[~, y1_ff] = ismember(ff,fr);
   % Signal_values_ff = P1(y1_ff);
    %Signal_peaks_ff = max(Signal_values_ff);
 %[~, Fund_Freq] = ismember(Signal_peaks_ff, P1);
    %FF_loc = fr(Fund_Freq); 
...
FF_loc = fr(ff_loc);   %location of the fundamental frequency 
 ...
 %nf = fr(fr > (FF_loc.*.05) & fr< (FF_loc.*.05)); %find first harmonic at half the Fundamental frequency
nf_t = (FF_loc/2);
nf = fr(fr>(nf_t - 0.3) & fr<(nf_t + 0.3));
...
%set(gca, 'YScale', 'log')
hold off
title('HR AP Single-Sided Amplitude Spectrum')
xlabel('f(Hz)')
ylabel('P1(f)')

%saveas(figure(2), 'HR AP.png')
