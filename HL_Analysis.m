function [HL_data] = HL_Analysis(v3d_data)
...
Hill_t = minus(HL_EN, HL_ST); %Rocky terrain time duration in seconds
HL_Rind = round((v3d_data.HL_indR./10)); %Right HS indices during rocky terrain
HL_Lind = round((v3d_data.HL_indL./10)); %Left HS indices during rocky terrain 
R_StepT = (HL_Rind./100); %Right step times during rocky terrain
L_StepT = (HL_Lind./100); %Left step times during rocky terrain
...
RFoot_loc = Rtemp_foot(HL_Rind,2);
Ltemp_foot = v3d_data.LFootPos{:,:};
LFoot_loc = Ltemp_foot(HL_Lind,2);
...
RLHL_HS = RLHL(HL_Rind,:); %raw right lateral heel trajectories at heel strikes 
RHEEL = cell2mat(v3d_data.c3d_rheel); %raw right heel trajectories 
RHEEL_HS = RHEEL(HL_Rind,:); %raw right heel trajectories at heel strikes  
LLHL = cell2mat(v3d_data.c3d_llhl); %raw left lateral heel trajectories 
LLHL_HS = LLHL(HL_Lind,:); %raw left lateral heel trajectories at heel strikes
LHEEL = cell2mat(v3d_data.c3d_lheel); %raw left heel trajectories 
LHEEL_HS = LHEEL(HL_Lind,:); %raw left heel trajectories at heel strikes
...
pCOG_RHS = pCOG(HL_Rind,:); %Center of Mass position at Right Heel Strikes 
pCOG_LHS = pCOG(HL_Lind,:); %Center of Mass position at Right Heel Strikes 
COG_Rind = HL_Rind - 1; %find the frames prior to right heel-strike to calculate instantenous acceleration 
COG_Lind = HL_Lind - 1; %find the frames prior to the left heel-strike to calculate instantaneous acceleration 
vCOG_prRHS = vCOG(COG_Rind,:);
vCOG_prLHS = vCOG(COG_Lind,:);
vCOG_RHS = vCOG(HL_Rind,:); %Center of Mass velocity at Right Heel Strikes 
vCOG_LHS = vCOG(HL_Lind,:); %Center of Mass velocity at Right Heel Strikes 
...
LFoot_loc_AP = Ltemp_foot(HL_Rind,2); % Left foot AP position data during Right heel-strike
RFoot_loc_AP = Rtemp_foot(HL_Rind,2); % Right AP foot position data during Right heel-Strike 
...
Ltemp_foot = v3d_data.LFootPos{:,:}; % get left foot position data
LFoot_loc_AP_LHS = Ltemp_foot(HL_Lind,2); % AP left foot position data @ LHS
RFoot_loc_AP_LHS = Rtemp_foot(HL_Lind,2); % AP right foot position data @ LHS
...
Ltemp_foot = v3d_data.LFootPos{:,:}; % get left foot position data
LFoot_loc_ML = Ltemp_foot(HL_Rind,1); % ML left foot position data
RFoot_loc_ML = Rtemp_foot(HL_Rind,1); % ML right foot position data
...
LFoot_loc_ML = temp_Lfoot(HL_Lind,1); % ML left foot position data at left HS
RFoot_loc_ML = Rtemp_foot(HL_Lind,1); % ML right foot position data at left HS 
temp_SW = abs(LFoot_loc_ML - RFoot_loc_ML); %temporary difference in ML foot position, the step width
...
HL_data.MOS_avg_RML = MOS_avg_RML;
HL_data.MOS_avg_RAP = MOS_avg_RAP;
HL_data.MOS_avg_LML = MOS_avg_LML;
HL_data.MOS_avg_LAP = MOS_avg_LAP;
HL_data.CoV_RStepT = CoV_RStepT;
HL_data.CoV_LStepT = CoV_LStepT;
HL_data.CoV_SW_RHS = CoV_SW_RHS;
HL_data.CoV_SW_LHS = CoV_SW_LHS;
HL_data.CoV_LSL = CoV_LSL;
HL_data.CoV_RSL = CoV_RSL;
HL_data.avg_SW_LHS = avg_SW_LHS;
HL_data.avg_SW_RHS = avg_SW_RHS;
HL_data.avg_LStepT = avg_LStepT;
HL_data.avg_RStepT = avg_RStepT;
HL_data.avg_LSL_LHS = avg_LSL_LHS;
HL_data.avg_RSL_RHS = avg_RSL_RHS;
HL_data.HR_AP = HR_AP;
HL_data.HR_ML = HR_ML;
HL_data.HR_Vert = HR_Vert;
HL_data.MOS_std_RML = MOS_std_RML;
HL_data.MOS_std_LML = MOS_std_LML;
HL_data.std_RStepT = std_RStepT;
HL_data.std_LStepT = std_LStepT;
HL_data.std_SW_RHS= std_SW_RHS;
HL_data.std_SW_LHS = std_SW_LHS;
HL_data.AngMmt_MLavg = AngMmt_MLavg; 
HL_data.AngMmt_MLstd = AngMmt_MLstd; 
HL_data.AngMmt_MLpk = AngMmt_MLpk;
HL_data.ANgMmt_MLpk_SD = AngMmt_MLpk_SD;
HL_data.AngMmt_APavg = AngMmt_APavg; 
HL_data.AngMmt_APstd = AngMmt_APstd;
HL_data.AngMmt_APpk = AngMmt_APpk;
HL_data.AngMmt_APpk_SD = AngMmt_APpk_SD;
HL_data.AngMmt_Vavg = AngMmt_Vavg;
HL_data.AngMmt_Vstd = AngMmt_Vstd;
HL_data.AngMmt_Vpk = AngMmt_Vpk;
HL_data.AngMmt_Vpk_SD = AngMmt_Vpk_SD;
HL_data.Trk_LV_MLavg = TrkLV_MLavg;
HL_data.Trk_LV_MLstd = TrkLV_MLstd;
HL_data.TrkLV_MLpk = TrkLV_MLpk;
HL_data.TrkLV_MLpk_SD = TrkLV_MLpk_SD;
HL_data.Trk_LV_APavg = TrkLV_APavg;
HL_data.Trk_LV_APstd = TrkLV_APstd;
HL_data.TrkLV_APpk = TrkLV_APpk;
HL_data.TrkLV_APpk_SD = TrkLV_APpk_SD;
HL_data.Trk_LV_Vavg = TrkLV_Vavg;
HL_data.Trk_LV_Vstd = TrkLV_Vstd;
HL_data.TrkLV_Vpk = TrkLV_Vpk;
HL_data.TrkLV_Vpk_SD = TrkLV_Vpk_SD;
HL_data.Trk_AV_MLavg = TrkAV_MLavg;
HL_data.Trk_AV_MLstd = TrkAV_MLstd;
HL_data.TrkAV_MLpk = TrkAV_MLpk;
HL_data.TrkAV_MLpk_SD = TrkAV_MLpk_SD;
HL_data.Trk_AV_APavg = TrkAV_APavg;
HL_data.Trk_AV_APstd = TrkAV_APstd;
HL_data.TrkAV_APpk = TrkAV_APpk;
HL_data.TrkAV_APpk_SD = TrkAV_APpk_SD;
HL_data.Trk_AV_Vavg = TrkAV_Vavg;
HL_data.Trk_AV_Vstd = TrkAV_Vstd;
HL_data.TrkAV_Vpk = TrkAV_Vpk;
HL_data.TrkAV_Vpk_SD = TrkAV_Vpk_SD;
HL_data.Hill_t = Hill_t;
HL_data.Larm_ROM = Larm_ROM;
HL_data.Rarm_ROM = Rarm_ROM;
