function [Rckdata] = Rck_Analysis(v3d_data)
...
%% Rocky Terrain Analysis %%
Rock_t = minus(Rock_EN, Rock_ST); %Rocky terrain time duration in seconds
Rck_Rind = round((v3d_data.Rck_indR./10)); %Right HS indices during rocky terrain
Rck_Lind = round((v3d_data.Rck_indL./10)); %Left HS indices during rocky terrain 
R_StepT = (Rck_Rind./100); %Right step times during rocky terrain
L_StepT = (Rck_Lind./100); %Left step times during rocky terrain
...
RFoot_loc = Rtemp_foot(Rck_Rind,2);
Ltemp_foot = v3d_data.LFootPos{:,:};
LFoot_loc = Ltemp_foot(Rck_Lind,2);
...
RLHL_HS = RLHL(Rck_Rind,:); %raw right lateral heel trajectories at heel strikes 
RHEEL = cell2mat(v3d_data.c3d_rheel); %raw right heel trajectories 
RHEEL_HS = RHEEL(Rck_Rind,:); %raw right heel trajectories at heel strikes  
LLHL = cell2mat(v3d_data.c3d_llhl); %raw left lateral heel trajectories 
LLHL_HS = LLHL(Rck_Lind,:); %raw left lateral heel trajectories at heel strikes
LHEEL = cell2mat(v3d_data.c3d_lheel); %raw left heel trajectories 
LHEEL_HS = LHEEL(Rck_Lind,:); %raw left heel trajectories at heel strikes
...
%% extract lateral heel markers
%RLHL = v3d_data.RLHL{:,:}(Rck_Rind,1:3); %right lateral heel marker during right heel-strike on rocky terrain
%RLHLt =  RLHL(all(~isnan(RLHL),2),:); %remove all non-numbers
%RLHL_raw = v3d_data.RLHL{:,:}(:,1); %call the raw right lateral heel marker data
%[~, Rck_Rindn] = ismember(RLHLt, RLHL_raw); %Find the indices of the remaining RLHL data after NaN are removed 
%Rck_Rindn = Rck_Rindn(:,1); 

%LLHL = v3d_data.LLHL{:,:}(Rck_Lind,1:3); %left lateral heel marker during left heel-strike on rocky terrain
%LLHLt = LLHL(all(~isnan(LLHL),2),:); %remove all non-numbers 
%LLHL_raw = v3d_data.LLHL{:,:}(:,1); %call the raw left lateral heel marker data
%[~, Rck_Lindn] = ismember(LLHLt, LLHL_raw); %find the indices of the ramining LLHL data after NaN are removed 
%Rck_Lindn = Rck_Lindn(:,1);
...
pCOG_RHS = pCOG(Rck_Rind,:); %Center of Mass position at Right Heel Strikes 
pCOG_LHS = pCOG(Rck_Lind,:); %Center of Mass position at Right Heel Strikes 
COG_Rind = Rck_Rind - 1; %find the frames prior to right heel-strike to calculate instantenous acceleration 
COG_Lind = Rck_Lind - 1; %find the frames prior to the left heel-strike to calculate instantaneous acceleration 
vCOG_prRHS = vCOG(COG_Rind,:);
vCOG_prLHS = vCOG(COG_Lind,:);
vCOG_RHS = vCOG(Rck_Rind,:); %Center of Mass velocity at Right Heel Strikes 
vCOG_LHS = vCOG(Rck_Lind,:); %Center of Mass velocity at Right Heel Strikes 
...
%% Angular Momentum 
Ang_Mom = v3d_data.ModelAngMmntm{:,:};%call angular momentum data
indext = COG_indeces(1:2:end); %isolate right foot indices to group the subsequent data into individual strides (stride = 2 steps [left and right])
idx =  cell(1,length(indext)-1); %create blank cell to store the time points for each individual stride 
for i = 1:(length(indext)-1); %isolate the time points for each stride, 1 cell = 1 stride
    idx{:,i} = indext(i:i+1); 
end 
...
for k = 1:length(AngMomt); %separate the angular momentum data into individual strides
...
Trk_LV_R = v3d_data.TrunkLinVel{:,:}; %call trunk linear velocity data
...
Trk_AV_R = v3d_data.TrunkAngVel{:,:}; %call trunk angular velocity data
...
LFoot_loc_AP = Ltemp_foot(Rck_Rind,2); % Left foot AP position data during Right heel-strike
RFoot_loc_AP = Rtemp_foot(Rck_Rind,2); % Right AP foot position data during Right heel-Strike 
...
Ltemp_foot = v3d_data.LFootPos{:,:}; % get left foot position data
LFoot_loc_AP_LHS = Ltemp_foot(Rck_Lind,2); % AP left foot position data @ LHS
RFoot_loc_AP_LHS = Rtemp_foot(Rck_Lind,2); % AP right foot position data @ LHS
...
CoV_LSL = (std_LSL_LHS/avg_LSL_LHS)*100; %Coefficient of Variation Left Step Length



%% Calculate Step Width Cov at Right Heel Strike 
Ltemp_foot = v3d_data.LFootPos{:,:}; % get left foot position data
LFoot_loc_ML = Ltemp_foot(Rck_Rind,1); % ML left foot position data
RFoot_loc_ML = Rtemp_foot(Rck_Rind,1); % ML right foot position data
...
LFoot_loc_ML = temp_Lfoot(Rck_Lind,1); % ML left foot position data at left HS
RFoot_loc_ML = Rtemp_foot(Rck_Lind,1); % ML right foot position data at left HS 
temp_SW = abs(LFoot_loc_ML - RFoot_loc_ML); %temporary difference in ML foot position, the step width
...
Rckdata.MOS_avg_RML = MOS_avg_RML;
Rckdata.MOS_avg_RAP = MOS_avg_RAP;
Rckdata.MOS_avg_LML = MOS_avg_LML;
Rckdata.MOS_avg_LAP = MOS_avg_LAP;
Rckdata.CoV_RStepT = CoV_RStepT;
Rckdata.CoV_LStepT = CoV_LStepT;
Rckdata.CoV_SW_RHS = CoV_SW_RHS;
Rckdata.CoV_SW_LHS = CoV_SW_LHS;
Rckdata.CoV_LSL = CoV_LSL;
Rckdata.CoV_RSL = CoV_RSL;
Rckdata.avg_SW_LHS = avg_SW_LHS;
Rckdata.avg_SW_RHS = avg_SW_RHS;
Rckdata.avg_LStepT = avg_LStepT;
Rckdata.avg_RStepT = avg_RStepT;
Rckdata.avg_LSL_LHS = avg_LSL_LHS;
Rckdata.avg_RSL_RHS = avg_RSL_RHS;
Rckdata.HR_AP = HR_AP;
Rckdata.HR_ML = HR_ML;
Rckdata.HR_Vert = HR_Vert;
Rckdata.MOS_std_RML = MOS_std_RML;
Rckdata.MOS_std_LML = MOS_std_LML;
Rckdata.std_RStepT = std_RStepT;
Rckdata.std_LStepT = std_LStepT;
Rckdata.std_SW_RHS= std_SW_RHS;
Rckdata.std_SW_LHS = std_SW_LHS;
Rckdata.AngMmt_MLavg = AngMmt_MLavg; 
Rckdata.AngMmt_MLstd = AngMmt_MLstd; 
Rckdata.AngMmt_MLpk = AngMmt_MLpk;
Rckdata.ANgMmt_MLpk_SD = AngMmt_MLpk_SD;
Rckdata.AngMmt_APavg = AngMmt_APavg; 
Rckdata.AngMmt_APstd = AngMmt_APstd;
Rckdata.AngMmt_APpk = AngMmt_APpk;
Rckdata.AngMmt_APpk_SD = AngMmt_APpk_SD;
Rckdata.AngMmt_Vavg = AngMmt_Vavg;
Rckdata.AngMmt_Vstd = AngMmt_Vstd;
Rckdata.AngMmt_Vpk = AngMmt_Vpk;
Rckdata.AngMmt_Vpk_SD = AngMmt_Vpk_SD;
Rckdata.Trk_LV_MLavg = TrkLV_MLavg;
Rckdata.Trk_LV_MLstd = TrkLV_MLstd;
Rckdata.TrkLV_MLpk = TrkLV_MLpk;
Rckdata.TrkLV_MLpk_SD = TrkLV_MLpk_SD;
Rckdata.Trk_LV_APavg = TrkLV_APavg;
Rckdata.Trk_LV_APstd = TrkLV_APstd;
Rckdata.TrkLV_APpk = TrkLV_APpk;
Rckdata.TrkLV_APpk_SD = TrkLV_APpk_SD;
Rckdata.Trk_LV_Vavg = TrkLV_Vavg;
Rckdata.Trk_LV_Vstd = TrkLV_Vstd;
Rckdata.TrkLV_Vpk = TrkLV_Vpk;
Rckdata.TrkLV_Vpk_SD = TrkLV_Vpk_SD;
Rckdata.Trk_AV_MLavg = TrkAV_MLavg;
Rckdata.Trk_AV_MLstd = TrkAV_MLstd;
Rckdata.TrkAV_MLpk = TrkAV_MLpk;
Rckdata.TrkAV_MLpk_SD = TrkAV_MLpk_SD;
Rckdata.Trk_AV_APavg = TrkAV_APavg;
Rckdata.Trk_AV_APstd = TrkAV_APstd;
Rckdata.TrkAV_APpk = TrkAV_APpk;
Rckdata.TrkAV_APpk_SD = TrkAV_APpk_SD;
Rckdata.Trk_AV_Vavg = TrkAV_Vavg;
Rckdata.Trk_AV_Vstd = TrkAV_Vstd;
Rckdata.TrkAV_Vpk = TrkAV_Vpk;
Rckdata.TrkAV_Vpk_SD = TrkAV_Vpk_SD;
Rckdata.Rock_t = Rock_t;
Rckdata.Larm_ROM = Larm_ROM;
Rckdata.Rarm_ROM = Rarm_ROM;
end 